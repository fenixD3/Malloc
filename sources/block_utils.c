	...
	const t_alloc_info *alloc_info)
{
	...
	blocks_meta = (t_block *)heap_block_shift(target_heap);
	...
	while (blocks_meta)
	{
			...
			&& blocks_meta->data_size >= alloc_info->block_size)
		{
			...
			return (block_data_shift(blocks_meta));
		}
	...
	const t_alloc_info *alloc_info)
{
	...
	if (!last_block)
		new_block = (t_block *)heap_block_shift(target_heap);
	else
		new_block = (t_block *)(block_data_shift(last_block)
	...
	return (block_data_shift(new_block));
}
